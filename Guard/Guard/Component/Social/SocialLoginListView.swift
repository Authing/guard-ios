...
open class SocialLoginListView: UIView, AttributedViewProtocol {
        ...
        if let connections: [NSDictionary] = config?.data?["ecConnections"] as? [NSDictionary] {
            ...
            for conn in connections {
                if let type = conn["type"] as? String {
                    ...
                    } else if ("amazon:mobile" == type) {
                        srcs.setValue("amazon", forKey: "21")
                    } else if ("twitter:mobile" == type) {
                        srcs.setValue("twitter", forKey: "22")
                    }
                }
            }
    ...
    class func handleSrc(container: UIView, _ sources: Array<String>, _ isVerticalLayout: Bool = false) {
        ...
        for s in srcs {
            ...
            } else if ("amazon" == trimmed) {
                if let type = Bundle(identifier: "cn.authing.Amazon")?.classNamed("Amazon.AmazonLoginButton") as? SocialLoginButton.Type {
                    let view = type.init()
                    if isVerticalLayout {
                        view.setTitle("authing_social_amazon".L, for: .normal)
                    }
                    container.addSubview(view)
                }
            } else if ("twitter" == trimmed) {
                if let type = Bundle(identifier: "cn.authing.TwitterSignIn")?.classNamed("TwitterSignIn.TwitterSignInButton") as? SocialLoginButton.Type {
                    let view = type.init()
                    if isVerticalLayout {
                        view.setTitle("authing_social_twitter".L, for: .normal)
                    }
                    container.addSubview(view)
                }
