...
open class OneAuth {
    ...
    public static func start(_ vc: UIViewController, businessId: String? = nil, model: Any? = nil, completion: @escaping(Int, String?, UserInfo?)->Void) {
        ...
        if let t = token {
            Util.getConfig(vc.view) { config in
                OneAuth.setCustomUI(config, vc, model, completion: completion)
            ...
            NTESQuickLoginManager.sharedInstance().getPhoneNumberCompletion { result in
                ...
                if (success) {
                    ...
                    DispatchQueue.main.async() {
                        Util.getConfig(vc.view) { config in
                            if let c = config {
                                OneAuth.setCustomUI(c, vc, model, completion: completion)
    ...
    static func setCustomUI(_ config: Config?, _ vc: UIViewController,_ model: Any?, completion: @escaping(Int, String?, UserInfo?)->Void) {
        
        if let md: NTESQuickLoginModel = model as? NTESQuickLoginModel {
            NTESQuickLoginManager.sharedInstance().setupModel(md)
        } else {
